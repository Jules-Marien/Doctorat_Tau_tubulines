...
selection_chain = "protein and not (name H*) and not segid PROD"
...
for t in range(len(matrices_contacts[:,1])):
    ...
    for i in range(1,len(groupe_2_indices_change)):
        ...
        for j in range(1,len(groupe_1_indices_change)):
            ...
            contact_per_residue_of_Tau = np.any( matrices_contacts_brut[t][boundary_left_1:boundary_right_1,boundary_left_2:boundary_right_2]  )
...
np.savetxt('nombre_total_de_contacts_modele'+modele+'.txt',nombre_contacts)
...
plt.title('Nombre de contacts résidu à résidu entre le fragment de Tau \n et les tubulines du modele '+modele+' au cours de la trajectoire')
...
plt.savefig('test_Plot_nbre_total_contacts_Tau_tubulines_modele17_avec_tau.png')
...
fraction_native_contacts = np.empty((len(matrices_contacts[:,1])))
fraction_native_contacts[0] = 1.

for t in range(1,len(matrices_contacts[:,1])):
    
    
    fraction_native_contacts[t] = nombre_contacts[t]/nombre_contacts[0]
