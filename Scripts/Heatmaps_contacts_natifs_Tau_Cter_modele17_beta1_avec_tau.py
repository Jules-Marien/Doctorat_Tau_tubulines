...
selection_chain = "around "+ str(radius_cutoff)+ " (segid PROD and resid 1340:1366 and not (name H*))"
...
plt.imshow(matrix_heatmap[:,:-len_Tau], cmap='Reds',vmin = 0, vmax =100)

plt.title('Pourcentage de contacts natifs conservés entre résidus du fragment Tau \n et ceux du complexe de tubuline du modele '+modele+' sur la durée de la trajectoire', fontsize=13,weight='bold')

plt.colorbar().set_label(label='% de contacts natifs conservés',fontsize=13,weight='bold')
...
plt.xlabel('Résidus du complexe en contact natif')
plt.ylabel('Résidus du fragment Tau')
    
plt.xticks(rotation=90)

plt.savefig('Heatmap_contacts_natifs_Tau_tubulines_modele'+modele+'.png')
...
plt.title('Pourcentage de contacts observés au sein des résidus du fragment Tau \n du modele '+modele+' sur la durée de la trajectoire', fontsize=13,weight='bold')
...
ax.xaxis.set_major_locator(mticker.FixedLocator(range(len_Tau)))
ax.xaxis.set_major_formatter(mticker.FixedFormatter(residues_name_Tau))

ax.yaxis.set_major_locator(mticker.FixedLocator(range(len_Tau)))
ax.yaxis.set_major_formatter(mticker.FixedFormatter(residues_name_Tau))
    

plt.xlabel('Résidus du fragment Tau')
plt.ylabel('Résidus du fragment Tau')
    
plt.xticks(rotation=90)

plt.savefig('Heatmap_contacts_Tau_Tau_modele'+modele+'.png')
